...
final class CustomiesBlockFactory {
	...
	public function registerBlock(Closure $blockFunc, string $identifier, ?Model $model = null, ?CreativeInventoryInfo $creativeInfo = null, ?Closure $serializer = null, ?Closure $deserializer = null): void {
        ...
        $stringCategory = $creativeInfo->getCategory();
        $PMCategory = CreativeInventoryInfo::getPMCategory($stringCategory);

        if (is_null($PMCategory)) {
            CreativeInventory::getInstance()->add($block->asItem());
        } else {
            $PMGroup = CreativeInventoryInfo::getPMGroup($stringCategory, $creativeInfo->getGroup());
            if (is_null($PMGroup)) {
                CreativeInventory::getInstance()->add($block->asItem(), $PMCategory);
            } else {
                CreativeInventory::getInstance()->add($block->asItem(), $PMCategory, $PMGroup);
            }
        }
